...
action :create do
  ...
  bash "Create Sentry@#{new_resource.server} organization <#{new_resource.organization}>member <#{new_resource.username}>" do
    code <<-EOH
      source #{virtualenv_path}/bin/activate
      python cli.py update organization "#{new_resource.organization}" --member #{new_resource.username}
      deactivate
    EOH
    cwd service_dir
